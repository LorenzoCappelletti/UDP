    ...
    public Server (int port)
    {
        try
        {
            ...
            while(true) // creazione del nostro ciclo infinito
            {
                    ...
                    InetAddress clientAddress = inPacket.getAddress(); //inPacket ha le informazioni del client per poi rispondere al mittente che poi diventa il destinatario
                    int clientPort = inPacket.getPort(); //inPacket ha le informazioni del client per poi rispondere al mittente che poi diventa il destinatario
        ...
        catch (IOException ex) // catch legato a  dSocket.receive(inPacket); 
        {
            Logger.getLogger(Server.class.getName()).log(Level.SEVERE, null, ex);
        }
